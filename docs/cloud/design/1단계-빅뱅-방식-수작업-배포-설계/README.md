...
- [배포 절차 다이어그램](#배포-절차-다이어그램)
- [의사결정](#의사결정)
- [배포 절차](#배포-절차)
- [도입 배경 및 한계](#도입-배경-및-한계-분석)
- [참고 자료](#참고-자료)
- [관련 문서](#관련-문서)
...
> TODO: 수작업 배포 흐름도 추가 예정 (로컬 빌드 → SCP 전송 → SSH 접속 → 서비스 재시작 → Health Check)

## 의사결정

각 항목의 상세 근거는 하위 문서 참고.

- **CSP**: AWS — 비용·익숙함 기반 선정. ([자세히 보기](./docs/인프라-의사결정/README.md))
- **프록시 / 웹서버**: NGINX — 설정 단순성·HTTPS 안정성·복구 속도 기준. ([자세히 보기](./docs/프록시-웹서버-선정/README.md))
- **인스턴스 사양**: EC2 t3.small — 피크 강한 트래픽 패턴 가정 + 버스터블 적합. ([자세히 보기](./docs/인스턴스-사양-선정/README.md))
...
- [인프라 의사결정](./docs/인프라-의사결정/README.md)
