...
- 어떤 CSP를 선택할 것인가 고려한 것 같은데, 결국 핵심은 **돈**이었음.
- 우리 서비스에서 비용이 그렇게까지 중요한 결정 요인이었나 — 다시 생각해볼 부분.
- 점수화 방식으로 비교했는데 정리 필요. 아마 **비용 + 익숙함** 두 축이었던 것 같음.
- **익숙함은 리스크가 될 수 있다**는 점도 고려해야 함.
- 가용 예산을 크레딧으로 잡아둔 것 — 그 이유에 대한 설명 보강 필요. (왜 크레딧 기반인가)
