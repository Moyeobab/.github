...
> 트래픽 기준: DAU 약 60명, MAU 약 500명, 월 약 18,000 요청
...
- 대역폭: 월 약 10GB (1TB 이내) → 초과 없음
- 빌드: 월 약 10시간 (400시간 이내) → 초과 없음
- **예상 비용: 약 $20/월**
...
- vCPU: 1 × $0.000463 × 60 × 24 × 30 = 약 $20/월
- RAM: 2 × $0.000232 × 60 × 24 × 30 = 약 $20/월
- **예상 비용: 약 $40/월** (Pro 포함 시 약 $60/월) + Vercel Pro $20 = **약 $80/월**
...
| --------------- | ------------- | ----------- | ------------------- |
| Pro 플랜        | -             | $20         | $20 크레딧 포함     |
| **Spring Boot** | 1 vCPU, 2GB   | 약 $40        | Backend (경합 없음) |
| **Next.js**     | 0.5 vCPU, 1GB | 약 $20        | Frontend (SSR 경량) |
| **PostgreSQL**  | 사용량 기반   | 약 $5-10      | DB                  |
| **총계**        | -             | **약 $85-90** | -                   |

**Railway Only 예상 비용 계산:**

- Pro 플랜: $20 (크레딧 $20 포함)
- Backend (Spring Boot): 1 vCPU 약 $20 + 2GB RAM 약 $20 = 약 $40
- Frontend (Next.js): 0.5 vCPU 약 $10 + 1GB RAM 약 $10 = 약 $20
- PostgreSQL: 사용량 기반 약 $5-10
- **총계: 약 $85-90/월** (크레딧 적용 전 기준)

**스펙 산정 근거:**

- Spring Boot: `-Xmx1g` 힙 + 네이티브 약 500MB = 약 1.5GB → 2GB면 충분
...
| 옵션                 | 구성                                    | 월 비용     | 스펙        | 비고                     |
| -------------------- | --------------------------------------- | ----------- | ----------- | ------------------------ |
| **AWS (Option A)**   | t4g.large (2 vCPU, 8GB) + gp3 100GB     | **약 $61**    | 충분        | 완전 제어, 단일 도메인   |
| **Vercel + Railway** | Vercel Pro + Railway Pro (Backend + DB) | **약 $80**    | 분리 최적화 | Frontend/Backend 분리    |
| **Railway Only**     | Next.js + Spring Boot + PostgreSQL      | **약 $85-90** | 분리 최적화 | 단일 플랫폼, 별도 도메인 |

**핵심 비교:**

| 항목          | AWS (약 $61)           | Railway (약 $85-90)      |
| ------------- | -------------------- | ---------------------- |
...
| ----------------- | ---------------------- | --------------------------- | -------------------- |
| 월 비용           | 약 $380 (24/7 상시 가동) | **약 $50-100** (상시 가동)    | 약 $15-50              |
...
| ------------- | --------- | ----------- | ---------------- | -------------------------------- |
| Flex Worker   | $0.00019  | 약 $0.68      | 약 $1-5 (저트래픽) | 사용 시만 과금, 콜드 스타트 있음 |
| Active Worker | $0.00013  | 약 $0.47      | **약 $343**        | 24/7 상시 가동, 최대 30% 할인    |
...
| 방안                   | 설명                                             | 예상 비용      |
| ---------------------- | ------------------------------------------------ | -------------- |
| **Flex (사용량 기반)** | 실제 추론 시간만 과금, 콜드 스타트 30초-2분 감수 | **약 $3-10/월**  |
| 샘플링 기반 운용       | 5-10% 요청만 모델로 처리                         | 약 $10-30        |
...
| -------------- | --------------- | ----------------- | ----------------------- |
| EC2 Frontend   | -               | t4g.small 약 $12    | t4g.small 약 $12          |
| EC2 Backend    | -               | t4g.small 약 $12    | t4g.small 약 $12          |
| RDS PostgreSQL | -               | db.t4g.micro 약 $15 | db.t4g.micro 약 $15       |
| EC2 (통합)     | t4g.large 약 $49  | -                 | -                       |
| EBS            | gp3 100GB 약 $11  | gp3 20GB x2 약 $5   | gp3 20GB x2 약 $5         |
| ALB            | -               | -                 | 약 $18                    |
| NAT Gateway    | -               | -                 | 약 $35                    |
| 데이터 전송    | 약 $1             | 약 $3               | 약 $5                     |
| **월 총계**    | **약 $61**        | **약 $47**          | **약 $102**               |

### D.3 종합 비교

| 항목          | Option A | Option B   | Option C      |
| ------------- | -------- | ---------- | ------------- |
| 월 비용       | 약 $61     | 약 $47       | 약 $102         |
| 관리 복잡도   | 낮음     | 중간       | 높음          |
...
| 항목        | 예상치       | 산정 근거                |
| ----------- | ------------ | ------------------------ |
| MAU         | 약 500명       | 초기 목표 사용자         |
| DAU         | 약 60명        | MAU의 12%                |
| 동시접속    | 3-6명        | DAU의 5-10%              |
| 일일 요청량 | 약 600 req/day | 사용자당 평균 10 req/day |
...
| ------------- | ---- | --- | --------------------- | ---------------- |
| **t4g.large** | 2    | 8GB | $0.0672               | **약 $49**         |

**구성 요소별 예상 리소스 사용량 (8GB 기준):**

| 구성 요소   | 예상 RAM    | 예상 CPU | 특성         | 비고                                |
| ----------- | ----------- | -------- | ------------ | ----------------------------------- |
| PostgreSQL  | 약 2GB        | 5-15%    | I/O Bound    | shared_buffers 512MB + OS 캐시 활용 |
| Spring Boot | 약 1.5GB      | 10-20%   | Memory Bound | -Xmx1g + 네이티브 약 500MB            |
| Next.js     | 약 700MB      | 5-10%    | Memory Bound | max_memory_restart: 850M            |
| Nginx       | 약 150MB      | < 1%     | -            | 리버스 프록시, TLS 처리             |
| OS/시스템   | 약 1GB        | 5%       | -            | 시스템 프로세스, 로깅               |
| **합계**    | **약 5.35GB** | **약 40%** |              | **여유 약 2.65GB**                    |

**pgvector 영향:**

- 예상 벡터 수: 약 1,400개 (768차원 기준)
- 메모리 사용: 약 5.6MB (무시 가능)
...
| ------------- | -------- | ---- | --- | ------- | -------------------------------------- |
| **t4g.large** | 버스터블 | 2    | 8GB | 약 $49    | 평상시 20% 기준선, 피크 시 100% 버스트 |
| m6g.large     | 고정     | 2    | 8GB | 약 $70    | 항상 100% 사용 가능                    |
| m7g.large     | 고정     | 2    | 8GB | 약 $75    | 항상 100% 사용 가능                    |
...
| ------------- | ---- | ---- | ------- | ---------------- | -------- |
| t4g.medium    | 2    | 4GB  | 약 $24    | 약 5.35GB          | OOM 발생 |
| **t4g.large** | 2    | 8GB  | 약 $49    | 약 5.35GB          | 약 2.65GB  |
| t4g.xlarge    | 4    | 16GB | 약 $98    | 약 5.35GB          | 오버스펙 |

**t4g.large (8GB) 선택 이유:**

1. **MVP에 적합한 비용 효율**

   - 현재 예상 사용량 약 5.35GB, 여유 약 2.65GB로 운영 가능
   - t4g.xlarge 대비 월 $49 절감 (약 50% 비용 절감)

2. **여유 메모리의 역할**

   - 약 2.65GB 여유 → OS 페이지 캐시로 활용 → DB 성능 향상
...
3. **메모리 압박**: 여유 약 2.65GB로 OS 캐시 제한적
...
| --------------------- | -------- | -------------- | ------------------ |
| 2시간 80% 집중        | 0.067    | 50-150 RPS     | **약 750배-2,200배** |
| **30분 50% 스파이크** | **0.17** | 50-150 RPS     | **약 300배-900배**   |
| 동시접속              | 3-6명    | 약 200명         | **약 33배-67배**     |
...
1. **RAM 8GB 선택**
   ...
   - 예상 사용량 약 5.35GB, 여유분 약 2.65GB 확보
   - 여유 메모리는 OS 페이지 캐시로 활용 → DB 성능 향상
   - 스왑 미설정 (OOM 발생 시 프로세스 종료로 빠른 문제 인지)

2. **CPU 버스트 크레딧 유지 가능**

   - DAU 60명, QPS < 1 기준 평균 CPU 사용률 약 20% 이하
...
| 항목         | 사양            | 월 비용     |
| ------------ | --------------- | ----------- |
| EC2 인스턴스 | t4g.large       | 약 $49        |
| EBS 스토리지 | gp3 100GB       | 약 $11        |
| 데이터 전송  | 10GB 아웃바운드 | 약 $1         |
| **총계**     |                 | **약 $61/월** |
...
| 단계      | 조건                             | 현재 → 변경                         | 월 비용 변화  |
| --------- | -------------------------------- | ----------------------------------- | ------------- |
| **0단계** | DAU 약 60                          | t4g.large (현재)                    | 약 $61          |
| **1단계** | 메모리 부족 또는 Blue-Green 필요 | t4g.large → t4g.xlarge (16GB)       | 약 $61 → 약 $110  |
| **2단계** | CPU 크레딧 부족                  | t4g.xlarge → m6g.xlarge (고정 성능) | 약 $110 → 약 $140 |
