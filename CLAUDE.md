...
- `main`에 직접 푸시 가능. 별도 PR 절차는 없다.
- **단, push는 항상 사용자 승인 후에만 실행한다.** 로컬 커밋은 자유롭게 만들어도 되지만, `git push` 직전에는 반드시 사용자에게 확인을 받는다.
- force push가 필요한 경우(예: 직전 커밋 메시지 수정)에도 동일하게 승인을 받고, `--force-with-lease`를 쓴다.
